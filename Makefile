...
#   make test-rust       - Rust target: parse/round-trip and end-to-end typecheck of the
#                          acceptance inventory (needs the private rust-spectec submodule;
...
.PHONY: test-rust test-rust-parse
...
# The Rust target: every program of the acceptance inventory parses and round-trips
# (Phase 4 design section 6, milestone (1)), and every one of them is typechecked end to end
# through Chapter 11's Pgm_ok and compared against the DOCUMENT's verdict, which is what
# expect.yaml carries.  The corpus is regenerated by rust-type-semantics'
# tools/run_encoding.py --parse-only; this target checks the committed copy.
#
# It is NOT part of `make test`, and its cram stanza is on an alias of its own, because it
# needs spectec/specs/rust -- the PRIVATE rust-spectec submodule.  A public clone deinits that
