...
# (Phase 4 design section 6, milestone (1)).  The corpus is regenerated by rust-type-semantics'
# tools/run_encoding.py --parse-only; this target checks the committed copy.
test-rust-parse:
	@echo "#### Running Rust target parse/round-trip tests"
	@$(DUNE) build --promote-install-files=false @test/package/rust/runtest --profile=release && echo OK
