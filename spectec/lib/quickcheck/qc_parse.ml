...
let try_parse_hint (grp : string) : Qc_ast.ast_hint option =
  let inner =
    ...
    if len >= 2 && grp.[0] = '(' && grp.[len - 1] = ')' then
      String.sub grp 1 (len - 2) |> String.trim
    else grp
  in
  let parts =
...
let parse_param_group (grp : string) : (Qc_ast.ast_param, string) result =
  ...
  | None -> Error (Printf.sprintf "param declaration missing ':' in '%s'" grp)
  | Some colon -> (
...
let parse_block (name : string) (block_lines : string list) :
      ...
      (fun acc grp ->
        ...
        | Ok (hint, params) -> (
            ...
            | Some h ->
                if hint <> None then
                  Error
                    (Printf.sprintf "quickcheck/%s: more than one hint" name)
  ...
  let rec collect_params params hint lines =
    ...
    | line :: rest -> (
        ...
        | L_Header _ ->
            Error
              (Printf.sprintf "quickcheck/%s: unexpected nested header" name)
        | L_Prem | L_Goal -> collect_body params hint None [] (line :: rest))
  and collect_body params hint goal prems lines =
    ...
    | line :: rest -> (
        ...
        | L_Param ->
            Error
              (Printf.sprintf
                 "quickcheck/%s: param declaration after body not allowed" name)
        | L_Prem -> (
        ...
        | L_Goal -> (
            ...
            | Some _ ->
                Error
                  (Printf.sprintf "quickcheck/%s: more than one goal line" name)
            | None -> (
...
let split_into_blocks (lines : string list) : (string * string list) list =
            ...
            (fun acc (name, _) ->
              ...
              | Ok seen ->
                  if List.mem name seen then
                    Error
                      (Printf.sprintf "quickcheck: duplicate block name '%s'"
                         name)
                  else Ok (name :: seen))
            (Ok []) blocks
        in
