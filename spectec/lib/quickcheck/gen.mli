(** Gen monad: size-aware random value generator.
    ...
    [newtype Gen a = Gen (Int -> Rand -> a)]. The size parameter ([int]) bounds
    the complexity of generated values, and [Random.t] carries the PRNG state.
    Both are propagated implicitly through bind. *)

(** Generator for ['a] values. Internally represented as
    [int -> Random.t -> 'a]. *)
type 'a t
...
(** Monadic bind: splits the PRNG state and supplies an independent stream to
    the continuation. Direct translation of [>>=] from goal.md. *)
...
(** [and*] runs two generators independently by splitting the PRNG, producing a
    pair. *)
...
(** [sample gen] runs with default size (5) and a self-initialized PRNG. Useful
    for debugging and interactive exploration. *)
val sample : 'a t -> 'a

(** [of_fun f] wraps function [f] as a generator. Used to construct generators
    directly from raw functions. *)
...
(** [elements xs] picks one element from [xs] uniformly. Raises
...
(** [variant v gen] perturbs the PRNG state by [v] before running [gen]. Used by
    [coarbitrary] to build function generators. Implements the
    [rands r !! (v+1)] pattern from goal.md. *)
...
(** [list_of gen] generates a list whose length is bounded by the size
    parameter. [~min] specifies the minimum length (default 0). *)
