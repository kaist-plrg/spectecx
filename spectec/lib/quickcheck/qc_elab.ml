...
let rec elab_plaintyp (pt : El.plaintyp) : typ = elab_plaintyp' pt.it $ pt.at
...
and elab_plaintyp' (pt' : El.plaintyp') : typ' =
  ...
  | El.VarT (id, targs) ->
      let il_targs = List.map (fun ta -> elab_plaintyp' ta.it $ ta.at) targs in
      VarT (id, il_targs)
...
let prepend_output_varpr_prems (spec : spec)
    (var_decls : (El.id * El.plaintyp) list) (prems : El.prem list) :
    El.prem list =
  ...
  List.iter
    (fun prem ->
      List.iter
        (fun (id, plaintyp) ->
          if not (List.mem id.it !known) then (
            known := id.it :: !known;
...
let elab_block (spec_il : spec) (block : Qc_ast.ast_block) :
  ...
  | Some goal -> (
      ...
      | Ok (il_prems, prems_output_vars) -> (
          ...
          in
          match
            Elaborate.elab_prems_in_spec spec_il var_decls all_with_decls
          with
          | Error e -> Error (Elaborate.error_to_string e)
          | Ok (il_all, _) ->
              let il_goal =
                ...
                | None ->
                    failwith
                      (Printf.sprintf
                         "quickcheck/%s: goal not found after elaboration" name)
                | Some g -> g
              in
              ...
              Ok
                ( Qc_ir.QcProp
                    { name; free_vars; generator; prems_rel; goal_rel },
                  [ prems_def; goal_def ] )))
  | None -> (
