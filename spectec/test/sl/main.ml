(** SL interpreter test - runs P4 typechecker using the SL interpreter *)
...
let run ~negative specdir includes exclude_dirs testdir =
  ...
  | Ok () -> ()
  | Error err ->
      Format.printf "Failed to run SL interpreter:\n  %s\n"
        (Runner.Error.string_of_error err)

let command =
  Command.basic ~summary:"run SL interpreter typing test"
