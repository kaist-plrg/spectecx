...
let rev_ ~at (typ : targ) (vs : Value.t list) : Value.t result =
  at |> ignore;
  Ok (Value.list typ (List.rev vs))

(* dec $concat_<X>((X* )* ) : X* *)

let concat_ ~at (typ : targ) (vss : Value.t list list) : Value.t result =
  at |> ignore;
  Ok (Value.list typ (List.concat vss))

(* dec $distinct_<K>(K* ) : bool *)

let distinct_ ~at (_typ : targ) (vs : Value.t list) : Value.t result =
  at |> ignore;
...
let assoc_ ~at (_typ_x : targ) (typ_y : targ) (key_query : Value.t)
    (pairs : (Value.t * Value.t) list) : Value.t result =
  at |> ignore;
  let value_opt =
    List.fold_left
